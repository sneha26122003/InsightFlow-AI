...
# ── Image to Text (OpenCV + Tesseract) ────────
...
st.markdown("**AI-powered Text Summarizer** — Text, Images, Documents")
st.divider()
...
with st.sidebar:
    ...
    method = st.selectbox(
        ...
        help="Extractive picks sentences. Abstractive generates new text."
    )
    ...
    num_sentences = st.slider(
        ...
        max_value=10,
        value=3,
        help="For extractive method only"
    )
    ...
    if st.button("Refresh Hardware Stats"):
        st.rerun()
    ...
    st.divider()
    st.caption(f"OS: {hw['os']}")
    st.caption(f"RAM Total: {hw['ram_total']} GB")
...
with tab1:
    ...
    # Sample texts
    sample = st.selectbox("Or load a sample:", [
        "None",
        "Technology News",
        "Science Article",
        "Business News"
...
# ════════════════════════════════════════════
#  TAB 2 — IMAGE INPUT (OpenCV)
# ════════════════════════════════════════════
with tab2:
    st.subheader("🖼️ Upload Image — Extract Text & Summarize")
    st.caption("OpenCV preprocesses the image, Tesseract extracts text, then AI summarizes!")

    uploaded = st.file_uploader(
        "Upload Image",
        type=["png", "jpg", "jpeg", "webp"],
        help="Upload any image with text"
    )
    ...
    if uploaded:
        ...
        with st.spinner("OpenCV processing image..."):
            try:
                ...
                with col2:
                    st.image(
                        processed_img,
                        caption="OpenCV Processed (Grayscale + Threshold)",
                ...
                if extracted_text and len(extracted_text) > 50:
                    if st.button("✨ Summarize Extracted Text", type="primary"):
                        with st.spinner("Summarizing..."):
                            ...
                            col1, col2 = st.columns(2)
                            col1.metric("Original Words",
                                       len(extracted_text.split()))
                            col2.metric("Summary Words",
                                       len(result["summary"].split()))
                else:
                    st.warning("Not enough text extracted from image. Try a clearer image!")

            except Exception as e:
                st.error(f"Image processing error: {e}")
                st.info("Make sure Tesseract is installed!")
...
with tab3:
    ...
    with col1:
        ...
        - **Speed:** ⚡ ~5ms
        - **ROUGE-L:** ~0.35
        - **Requirements:** No download needed
    ...
    st.divider()
    st.subheader("📊 ROUGE Scores Comparison")

    import pandas as pd
    df = pd.DataFrame({
        "Model": ["BART-large-CNN", "T5-base", "Extractive (3 sent)"],
        "ROUGE-1": [0.64, 0.56, 0.48],
        "ROUGE-2": [0.52, 0.45, 0.38],
        "ROUGE-L": [0.48, 0.42, 0.35],
    })
    ...
    col1.metric("PyTorch Version", hw['torch_version'])
    col2.metric("Python Version", hw['python_version'])
    col3.metric("GPU Available", "Yes ✅" if hw['gpu_available'] else "No ❌")